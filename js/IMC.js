document.getElementById('form_PesoIdeal').addEventListener('submit', function(event) {
    event.preventDefault(); //event.preventDefault previene que el formulario se envíe y la pagina se recargue
    ...
    //Validamos que se llene todo el formulario
    if (isNaN(estatura) || isNaN(pesoActual) || isNaN(edad)) {
        alert("Por favor, termine de llenar el formulario.")
    } else {
        //calculamos el peso ideal según el sexo y la estatura
