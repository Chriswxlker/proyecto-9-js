...
function años() {
    ...
    //Validamos que el formulario se llene por completo
    if (isNaN(edad)) {
        alert("Por favor, ingrese su edad.")
    } else {
        //utilizar if para determinar parámetros
        if (edad <= 12) {
...
function IMC() {
    ...
    if (isNaN(peso) || isNaN(estatura)) {
        ...
        //Utilizamos if para determinar los parámetros
        if (imc < 18.5) {
...
function pA() {
    ...
    //Validamos que el formulario se llene por completo
    if (isNaN(presionArterial)) {
        alert("Por favor, ingrese su presión arterial.")
    } else {
        //Utilizamos if para determinar los parámetros
        if (presionArterial < 90) {
            document.getElementById("resultadoPresionArterial").innerHTML = "<div class= 'alert alert-primary' role='alert'><strong>Su clasificación es:</strong> Baja.</div>";

        } else if (presionArterial < 120){
            document.getElementById("resultadoPresionArterial").innerHTML = "<div class= 'alert alert-success' role='alert'><strong>Su clasificación es:</strong> Normal.</div>";

        } else if (presionArterial >= 120 && presionArterial <= 139){
            document.getElementById("resultadoPresionArterial").innerHTML = "<div class= 'alert alert-warning' role='alert'><strong>Su clasificación es:</strong> Pre hipertensión.</div>";

        } else if (presionArterial >= 140 && presionArterial <= 159){
            document.getElementById("resultadoPresionArterial").innerHTML = "<div class= 'alert alert-danger' role='alert'><strong>Su clasificación es:</strong> ALTA: Hipertensión fase 1.</div>";

        }  else if (presionArterial >= 160){
            document.getElementById("resultadoPresionArterial").innerHTML = "<div class= 'alert alert-danger' role='alert'><strong>Su clasificación es:</strong> ALTA: Hipertensión fase 2.</div>";
...
function peso_altura(){
    ...
    if (isNaN(edad2)) {
        ...
        if (sexo == "Hombre" || sexo == "hombre" && edad2 >= 11 && edad2 <= 14) {
            ...
            document.getElementById("resultado2").innerHTML = "<div class='alert alert-danger' role='alert'><strong>Error:</strong> Por faror, escriba su sexo.</div>";
        }
    }
}